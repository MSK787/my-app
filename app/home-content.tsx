  ...
  return (
    <section>
      <div className="mb-6 flex items-end justify-between">
        ...
        <Link
          href="/products"
          className="hidden text-sm font-semibold text-amber-700 hover:text-amber-800 sm:block"
        >
          {t("home.viewAll")}
        </Link>
      </div>
      ...
      <div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <Link
            ...
            className="rounded-full bg-amber-500 px-6 py-3 text-sm font-semibold text-slate-900 transition hover:bg-amber-400 active:scale-[0.98]"
          >
            {t("home.viewProduct")}
          </Link>
          <Link
...
function QuoteSection() {
  ...
  return (
    <section className="rounded-3xl bg-slate-900 px-6 py-12 text-center sm:px-12">
      ...
      <Link
        href="/contact"
        className="mt-6 inline-block rounded-full bg-amber-500 px-6 py-3 text-sm font-semibold text-slate-900 transition hover:bg-amber-400 active:scale-[0.98]"
      >
        {t("home.askQuote")}
      </Link>
    </section>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:py-24">
          <div className="animate-fade-up">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="/products"
                className="rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-amber-700 active:scale-[0.98]"
              >
                {t("home.shopAll")}
              </Link>
              <Link
          ...
          <div className="relative hidden md:block animate-fade-up [animation-delay:150ms]">
            <div className="grid grid-cols-2 gap-3">
              {HERO_IMAGES.map((image) => (
                <div
                  key={image.src}
                  className="relative aspect-square overflow-hidden rounded-3xl border-4 border-white bg-slate-100 shadow-xl shadow-amber-200/50"
                >
                  <Image
                    src={image.src}
                    alt={image.alt}
            ...
            <div className="animate-float absolute -bottom-4 -start-4 rounded-2xl border border-amber-200 bg-white px-5 py-3 shadow-lg">
